...
template <typename K, typename M, typename H>
HashMap<K, M, H>::HashMap(const HashMap<K, M, H>& other) : _size(0), _hash_function(other._hash_function), _buckets_array(other.bucket_count()) {
    for (auto curr : other._buckets_array) {
        while (curr != nullptr) {
            insert(curr->value);
            curr = curr->next;
        }
...
template <typename K, typename M, typename H>
HashMap<K, M, H>::HashMap(HashMap<K, M, H>&& other) : _size(other._size), _hash_function(std::move(other._hash_function)), _buckets_array(std::move(other._buckets_array)) {}
